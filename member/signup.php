...
<div class="sign_container">
    <div class="signup_sort">
        <form name="frmsign">
            ...
            <div class="campus">
                <label for="signcam">학교 이메일</label>
                <div class="campus_position">
                    <input type="text" placeholder="학교 이메일" id="signcam" name="m_be" class="m_be">
                    <input type="button" value="인증번호 발송" id="email_btn" onclick="cert_btn();">
                    <input type="hidden" name="email_check" id="email_check" value="0">
                </div>
            </div>
            <div class="campus" id="mailAuthLayout" style="display: none;">
                <label for="signcam">인증번호 입력</label>
                <div class="campus_position">
                    <input type="text" placeholder="인증번호 입력" id="mailAuthNum" name="mailAuthNum" class="mailAuthNum">
                    <input type="button" value="인증하기" id="email_btn" onclick="mailAuthCheck();">
                </div>
            </div>
            ...
            <div class="gender_ch">
                ...
                <select name="gender_chos" id="gender" class="m_ph1">
...
<script>
    function onSubmitAjax() {
        ...
        let email_check = $('#email_check');            //중복확인
    ...
    function cert_btn() {
        const emailcheck = $('#signcam').val().slice(-5, $('#signcam').val().length);
        if (emailcheck != 'ac.kr'){
            alert("학교 이메일만 회원가입이 가능합니다.");
            $('#signcam').val('');
        }else{
            var emailData = {
                email: $('#signcam').val(),
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result.result == "SUCCESS"){
                        alert("메일을 발송했습니다.");
                        $('#mailAuthLayout').css('display', 'block');
                    }else{
                        alert(result.result);
                    }
                },
    ...
    function mailAuthCheck() {
        $.ajax({
            type: 'post',
            dataType: 'json',
            url: './mailAuthCheck.php',
            data: { email: $('#signcam').val(),
                    authNum: $('#mailAuthNum').val() },
            success: function (data) {
                alert(data.result);
                if (data.result == "SUCCESS"){
                    alert("인증이 완료되었습니다.");
                    $('#email_check').val('1');
                } else if (data.result == "DB_FAIL") {
                    alert("DB저장 실패.");
                }else if (data.result == "NO_DATA") {
                    alert("인증번호 발송 버튼을 먼저 눌러주세요.");
                }
            },
            error: function (request, status, error) {
                console.log('code: '+request.status+"\n"+'message: '+request.responseText+"\n"+'error: '+error);
