...
<div class="container">
    <div class="buy_top">
        <ul>
            <li><a href="buy_ring.php" target="_self">초코링구매</a></li>
            <li><a href="buy_history.php" target="_self">구매내역</a></li>
            <li><a href="buy_use.php" target="_self">사용내역</a></li>
        </ul>
    </div>
    <?php
        $user_id = $_SESSION['user_id'];
    ...
    $sql = "SELECT * FROM choco where user_id='$user_id' and type='get' order by date DESC";
    $result = mysqli_query($conn, $sql);
    if (mysqli_num_rows($result) > 0){ //if 시작

        while ($row = mysqli_fetch_array($result)){ //while 시작
            ?>
            <div class="history_content1">
                <p class="b_h_date"><?=$row['date']?></p>
                <p class="b_h_purchase">구매: <?=$row['amounts']?>개</p>
                <p class="b_h_remain">결제방법: <?=$row['way']?></p>
                <p class="b_h_price">금액: <?=number_format($row['cost'])?>원</p><!--<span class="history_cancel">취소불가</span>-->
            </div>
            <?php
        } //while 끝
