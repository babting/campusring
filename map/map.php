...
$checkValue = $_POST['checkRow'];
if (empty($checkValue)){
    echo "<script>alert('멘토를 선택해주세요.'); location.href = '/map/map_select.php';</script>";
    exit;
}
//print_r($checkValue);
?>
...
<div class="container">
    ...
    <section>
        <div class="map_wrap">
            ...
            <ul id="my_spot">
                <!--                <button class="btn_myspot" type="button" onclick="getLocation();">내 위치</button>-->
            </ul>
        </div>
        ...
        <script>
            ...
            // 마커 하나를 지도위에 표시합니다
            <?php
            foreach ($checkValue as $user_id) {
            $sql = "select * from location where user_id = '$user_id' ";
            $result = mysqli_query($conn, $sql);
            $row = mysqli_fetch_array($result);
            ?>
            addMarker2(new kakao.maps.LatLng(<?=$row['latitude']?>, <?=$row['longitude']?>), '<?=$user_id?>' );
            <?php
            }
            ...
            function addMarker2(position, user_id) {
                ...
                var iwContent = '<a style="padding:5px;" href="/member/mentorpf.php?m_id='+user_id+'">'+ user_id + '</a>', // 인포윈도우에 표출될 내용으로 HTML 문자열이나 document element가 가능합니다
